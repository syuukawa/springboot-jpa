...
//https://blog.csdn.net/qing_mei_xiu/article/details/80283115
public class HttpClientsUtil {
    ...
    static class GetRunnable implements Runnable {
        ...
        private String url;
        private List<BqiTickerTable> bqiTickerTables = new ArrayList<>();

        public GetRunnable(CountDownLatch countDownLatch, String url, List<BqiTickerTable> bqiTickerTables) {
            this.countDownLatch = countDownLatch;
            this.url = url;
            this.bqiTickerTables = bqiTickerTables;
        }

        @Override
        public void run() {
            try {
                BqiTickerTable returnResult = null;
                String result = HttpClientsUtil.get(url);
                System.out.println(HttpClientsUtil.get(url));
                Gson gson = new Gson();
                returnResult = gson.fromJson(result.replace("[", "").replace("]", ""), BqiTickerTable.class);
                bqiTickerTables.add(returnResult);
            } finally {
...
//        }
        ...
        List<BqiTickerTable> bqiTickerTables = new ArrayList<>();
        ExecutorService executors = Executors.newFixedThreadPool(10);
        CountDownLatch countDownLatch = new CountDownLatch(10);
        for (int i = 0; i < 10; i++) {
            String url = "https://public.bqi.com/public/v1/ticker?code=" + "bitcoin";
            url = url + "&convert=CNY&start=0&limit=1&isKLine=1";
            HttpClientsUtil.get(url);
            // 启动线程获取数据
//            executors.execute(new GetRunnable(countDownLatch,url,bqiTickerTables));
        }
        System.out.println("bqiTickerTables Size ->" + bqiTickerTables.size());
...
//        }
//        System.out.println("consume No pool -> " + (System.currentTimeMillis() - stime)+" ms");
